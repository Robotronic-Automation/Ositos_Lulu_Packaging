...
void Ultrasonidos_Task( void * parameter )
{
  ...
  Buffer_Circ_Measure_t * buff = (Buffer_Circ_Measure_t *) parameter;
...
void Controlador_Task( void * parameter )
{
  uint32_t cm;
  TickType_t xLastWakeTime;
  Buffers_t * buffers = (Buffers_t *) parameter;
...
void GestorComMQTT_Task( void * parameter )
{
  Msg_MQTT_t msg_to_publish;
  TickType_t xLastWakeTime;
  Buffer_Circ_MQTT_t * buff = (Buffer_Circ_MQTT_t *) parameter;
  ...
  while (!PARAR || button_pressed)
  {
    ...
    if(get_item(&msg_to_publish, buff) == 0)
    {
      ...
      else if(strcmp(msg_to_publish.topic, TOPIC_PARADA_EMERGENCIA) == 0)
      {
        JsonDocument doc;
        doc["parada_emergencia"] = msg_to_publish.msg;
        String msg_json;
        serializeJson(doc, msg_json);
        enviarMensajePorTopic(msg_to_publish.topic, msg_json);
        break;
...
void Led_Task( void * parameter)
{
  char color[10];
  TickType_t xLastWakeTime;
  Var_Prot_String_t * var_prot = (Var_Prot_String_t *) parameter;
...
void Motor_Task( void * parameter)
{
  bool status;
  TickType_t xLastWakeTime;
  Var_Prot_Bool_t * var_prot = (Var_Prot_Bool_t *) parameter;
